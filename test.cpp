...
TEST(Nefroida, sample_test_2)
{
    Nefroida *object = new Nefroida;
    object->set_r(5.0);
    EXPECT_EQ(object->radius_of_curvature(2), abs(3 * 5.0 * sin(2)));
    delete object;
}
...
TEST(Nefroida, sample_test_4)
{
    Nefroida *object = new Nefroida;
    object->set_r(5.0);
    EXPECT_EQ(object->x_depending_on_t(2), 6 * 5.0 * cos(2) - 4 * 5.0 * cos(3 * 2));
    delete object;
}
...
TEST(Nefroida, sample_test_5)
{
    Nefroida *object = new Nefroida;
    object->set_r(5.0);
    EXPECT_EQ(object->y_depending_on_t(2), 4 * 5.0 * pow(sin(2), 3));
    delete object;
}
